...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>>{
    ...
    let tableRows = vec![Row::new(vec!["row11", "row12", "row13"]), 
    ...
    terminal.draw(|f| {
        ...
        let table = Table::new(tableRows.clone())
            ...
            .highlight_symbol(">>");
        f.render_stateful_widget(table, size, &mut tableState);
    })?;
    let input = rx.recv()?;
        // process input
        match input {
            ...
            Key::Down => {
                next(&mut tableState, &tableRows);
            }
            Key::Up => {
                previous(&mut tableState, &tableRows);
            }
            _ => {}
        }
